package com.video.qinxiang.app.views.custom;
...
public class SuperSwipeRefreshLayout extends ViewGroup {
    ...
    private static final int SPINNER_FINAL_OFFSET = 80;
    ...
    private int lastRefreshMark = -1;

    private boolean firstMoveInQueue = true; //一组事件中第一个move事件
    ...
    public void setHeaderView(View child) {
        ...
        time = (TextView) child.findViewById(R.id.tv_date);
    }
    ...
    public SuperSwipeRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mSpinnerFinalOffset = SPINNER_FINAL_OFFSET * metrics.density;
...
//        Log.d(LOG_TAG, "debug:onLayout childHeight = " + childHeight);
    ...
    public boolean isChildScrollToBottom() {
        ...
        if (mTarget instanceof RecyclerView) {
            ...
            if (lastPos > 0 && count > 0 && lastPos == count - 1) {
                //TODO guo
                View temp = absListView.getChildAt(absListView.getChildCount() - 1);
                if (temp != null) {
                    if (temp.getBottom() == absListView.getHeight()) {
                        return true;
                    }
                }
            }
    ...
    private boolean handlerPullTouchEvent(MotionEvent ev, int action) {
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                //TODO guo
                if (lastRefreshMark != -1 && firstMoveInQueue) {
                    int currentMark = (int) (System.currentTimeMillis() / 1000);
                    formatTime(time, currentMark - lastRefreshMark);
                    firstMoveInQueue = false;
                    Log.e(LOG_TAG, "lastRefreshMark  = " + lastRefreshMark);
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                if (overscrollTop > mTotalDragDistance) {
                    ...
                    lastRefreshMark = (int) (System.currentTimeMillis() / 1000);
                } else {
                    mRefreshing = false;
     ...
     * @param time
     */
    private void formatTime(TextView view, int time) {
        int time_day = 24 * 60 * 60;
        int time_hour = 60 * 60;
        int time_min = 60;
        Log.e(LOG_TAG, "时间差 = " + time);
        if (time / (time_day) > 1) {
            view.setText(time / time_day + "天前");
        } else if (time / (time_hour) > 1) {
            view.setText(time / (time_hour) + "小时前");
        } else if (time / time_min > 1) {
            view.setText(time / time_min + "分钟前");
        } else {
            view.setText(time + "秒钟前");
        }
    }
